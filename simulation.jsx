...
export default function RaceTrack() {
  ...
  const individualRacer = [
    { id: 'verstappen', flag: '🇳🇱', driverName: 'M. Verstappen', color: '#0600ef', delay: 0 },
  ];

  const racers = activeTab === 'individual' ? individualRacer : allRacers;
  const maxLaps = activeTab === 'individual' ? 3 : 50;
    ...
    const sorted = [...racers]
      .map((racer) => {
        ...
        return {
          ...
          // Progress is primarily lap-based, with pointIndex as a tiebreaker
          progress: pos.lap * 1000 + pos.pointIndex, // Multiply lap by a large number to prioritize it
        };
      })
      .sort((a, b) => {
        // Sort by lap first (descending), then by pointIndex (descending)
        if (b.lap !== a.lap) {
          return b.lap - a.lap;
        }
  ...
  // Reset positions when tab changes
  useEffect(() => {
    setPositions({});
    setRankings([]);
